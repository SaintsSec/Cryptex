...
class SE(Cipher): #make sure you change this from text to your cipher

    name = 'Static Encryption' #change the name
    type = 'cipher'
    ...
    defaultCheck = lambda val, default : val if val else default

    @staticmethod
    def encode(args):
        text = args.text
        image_width = SE.defaultCheck(args.imageWidth, 1)
        monocromatic = SE.defaultCheck(args.monocromatic, False)
        output = SE.defaultCheck(args.output, 'test.png')
    ...
    def decode(args):
        monocromatic = SE.defaultCheck(args.monocromatic, False)
        input = SE.defaultCheck(args.input, 'test.py')
        ...
        for _, height in enumerate(pixels): # Vertical
            if monocromatic:
                ...
                try:
                    ...
                    return {'text': f"Failed to decrypt image {input}, maybe you forgot to enable monocromatic mode.\n{e}", 'success': False}
